...
export class Solver {
  ...
  private tryResolve(node: ASTNode): boolean {
    ...
    const groupId = node.name === "group" ? undefined : (constraints.group || node.properties.group);
    ...
    if (node.name === "group") {
      ...
      this.resolved.set(id, { type: node.name, id, properties: { ...node.properties, x, y }, bounds: { x, y, width: 0, height: 0 } });
      return true;
    }

    // Miscellaneous label/note logic.
    if (node.name === "label" || node.name === "note" || node.name === "text") {
      const anchorRef = constraints.at || constraints.origin || constraints.connects;
      if (anchorRef) {
        if (!this.resolved.has(anchorRef.id)) return false;
        const anchorPos = this.getAnchorPosition(anchorRef.id, anchorRef.anchor);
        x = anchorPos.x;
        y = anchorPos.y;
      } else {
        x = constraints.x !== undefined ? constraints.x : (node.properties.x || 0);
      ...
      this.resolved.set(id, this.offsetComponent({ type: "label", id, properties: { ...node.properties, x, y }, bounds: { x, y, width: 0, height: 0 } }, groupOffset));
      return true;
    }
    ...
    if (node.name === "floor") {
      ...
      x = -width / 2; 
      this.resolved.set(id, this.offsetComponent({ type: node.name, id, properties: node.properties, bounds: { x, y, width, height } }, groupOffset));
      return true;
    }
    ...
    if (node.name === "block") {
      ...
      if (!resolvedY && onConstraint) {
        ...
        if (onConstraint.anchorRef.anchor === "surface") {
          if (target.type === "incline") {
            ...
            this.resolved.set(id, this.offsetComponent({
              type: node.name, id, properties: this.applyContactProperties(node.properties, onConstraint),
      ...
      this.resolved.set(id, this.offsetComponent({ type: node.name, id, properties: this.applyContactProperties(node.properties, onConstraint), bounds: { x, y, width, height } }, groupOffset));
      return true;
    }
    ...
    if (node.name === "incline") {
      ...
      const x0 = constraints.x !== undefined ? constraints.x : (node.properties.x || 0);
      let y0 = constraints.y !== undefined ? constraints.y : (node.properties.y || 0);
      if (onConstraint) {
      ...
      this.resolved.set(id, this.offsetComponent({ 
        type: node.name, id, properties: this.applyContactProperties({ ...node.properties, x: x0, y: y0 }, onConstraint),
        bounds: { x: minX, y: minY, width: Math.abs(base), height: Math.abs(h) },
        rotation: angle
      }, groupOffset));
      return true;
    }
    ...
    if (node.name === "pulley") {
      ...
      if (!resolvedX && !resolvedY && constraints.from) {
        ...
        if (!this.resolved.has(anchorRef.id)) return false;
        const anchorPos = this.getAnchorPosition(anchorRef.id, anchorRef.anchor);
        let angle = constraints.direction !== undefined ? constraints.direction : 0;
        if (constraints.relative_to) {
          const relative = this.resolved.get(constraints.relative_to.id);
          if (!relative) return false;
          angle += relative.rotation || 0;
        }
        const distance = constraints.distance !== undefined ? constraints.distance : 0;
        const rad = angle * Math.PI / 180;
      ...
      if (!resolvedX && constraints.x !== undefined) { x = constraints.x; resolvedX = true; }
      if (!resolvedY && constraints.y !== undefined) { y = constraints.y; resolvedY = true; }
      if (!resolvedX) x = 0;
      if (!resolvedY) y = 0;
      // For pulley, x, y is center. Bounds are [x - r, y - r, 2r, 2r]
      this.resolved.set(id, this.offsetComponent({ type: node.name, id, properties: node.properties, bounds: { x: x - r, y: y - r, width, height } }, groupOffset));
      return true;
    }

    // Connectors (Spring)
    if (node.name === "spring" || node.name === "string" || node.name === "rod" || node.name === "vector") {
      let canResolve = true;
      let endpoints: {x1:number,y1:number,x2:number,y2:number} | undefined = undefined;
      ...
      if (constraints.connects) {
        if (Array.isArray(constraints.connects)) {
          ...
          const hasRouting = constraints.connects.some(n => n.type === 'routing');
          ...
          if (!hasRouting && constraints.connects.length === 2) {
            ...
            if (!this.resolved.has(start.id) || !this.resolved.has(end.id)) {
              ...
              try {
                ...
                let y1 = p1.y;
                let y2 = p2.y;
                if (constraints.align === 'horizontal' || node.properties.align === 'horizontal') {
                  y1 = y2;
                }
            ...
            if (canResolve) {
               try {
                 ...
                 const startP = this.getAnchorPosition(startNode.id, startNode.anchor);
                 ...
                 for (let i = 1; i < constraints.connects.length; i++) {
                   ...
                   if (cnode.type === 'anchor') {
                     ...
                     const C = { x: pulleyNode.bounds!.x + r, y: pulleyNode.bounds!.y + r };
                     ...
                     if (isVover) {
                       const clampedX = Math.max(C.x - r, Math.min(C.x + r, currentP.x));
                       const ey = C.y + Math.sqrt(Math.max(0, r * r - (clampedX - C.x) ** 2));
                     ...
                     if (nextNode.type === 'anchor') {
                       nextTargetP = this.getAnchorPosition(nextNode.id, nextNode.anchor);
                     } else if (nextNode.type === 'routing') {
                       const nextPulley = this.resolved.get(nextNode.id)!;
                       const nextR = nextPulley.properties.radius || 1;
                       nextTargetP = { x: nextPulley.bounds!.x + nextR, y: nextPulley.bounds!.y + nextR };
                     } else {
                     ...
                     if (isVover) {
                       const clampedX = Math.max(C.x - r, Math.min(C.x + r, nextTargetP.x));
                       const ey = C.y + Math.sqrt(Math.max(0, r * r - (clampedX - C.x) ** 2));
                     ...
                     pathSegments.push({ type: 'arc', x: T_out.x, y: T_out.y, r, cx: C.x, cy: C.y, method: baseMethod, inP: T_in, outP: T_out });
                     currentP = T_out;
                   }
                 }
               } catch (e) {
          ...
          if (!this.resolved.has(start.id)) {
            ...
            try {
              const p1 = this.getAnchorPosition(start.id, start.anchor);
              let length = node.properties.length !== undefined ? node.properties.length : 2;
              let angle = node.properties.angle !== undefined ? node.properties.angle : 0;
              if (constraints.direction !== undefined) angle = constraints.direction;
              if (node.properties.direction !== undefined) angle = node.properties.direction;
              if (constraints.relative_to) {
                const relative = this.resolved.get(constraints.relative_to.id);
                if (!relative) return false;
                angle += relative.rotation || 0;
      ...
      if (canResolve) {
        this.resolved.set(id, this.offsetComponent({ type: node.name, id, properties: node.properties, endpoints, pathSegments, startX, startY }, groupOffset));
        return true;
      }
      return false;
    }
    ...
    this.resolved.set(id, this.offsetComponent({ type: node.name, id, properties: node.properties }, groupOffset));
    return true;
  }

  private getAnchorPosition(targetId: string, anchorName: string): { x: number, y: number } {
    const target = this.resolved.get(targetId);
    if (!target || !target.bounds) throw new Error(`Cannot find anchor ${targetId}.${anchorName}`);
    ...
    if (target.type === "incline") {
      ...
      if (anchorName === 'top' || anchorName === 'peak' || anchorName === 'crest') return { x: peakX, y: peakY };
      if (anchorName === 'base' || anchorName === 'bottom' || anchorName === 'origin') return { x: px, y: py };
      if (anchorName === 'center') return { x: (px + peakX) / 2, y: (py + peakY) / 2 };
      if (anchorName === 'surface') return { x: (px + peakX) / 2, y: (py + peakY) / 2 };
      return { x: peakX, y: peakY }; // default: peak
    }

    if (target.type === "floor") {
       cx = x + width / 2; // Center of the floor
       if (anchorName === "surface" || anchorName === "top") return { x: cx, y: y + height };
    }
    ...
    if (target.type === "wall") {
      ...
      if (anchorName === "right" || anchorName === "surface") return { x: x + width, y: cy };
    }
    
    if (target.type === "ceiling") {
       cx = x + width / 2;
       if (anchorName === "surface" || anchorName === "bottom") return { x: cx, y: y };
    }
    
    if (target.type === "pulley") {
       cx = x + width / 2;
       if (anchorName === "center") return { x: cx, y: cy };
       if (anchorName === "surface" || anchorName === "top") return { x: cx, y: cy + width/2 };
    ...
    if (target.type === "block") {
      ...
      const top = y + height;
      if (anchorName === "center") return { x: cx, y: cy };
      if (anchorName === "surface" || anchorName === "top") return rotatePoint(cx, top);
  ...
  private getTangents(A: {x: number, y: number}, C: {x: number, y: number}, r: number) {
    const dx = C.x - A.x;
    const dy = C.y - A.y;
  ...
  private shouldApplyGroupOffset(groupId: string | undefined, constraints: Record<string, any>): boolean {
  ...
  private getConstraintRefs(value: any): Array<{ id: string, anchor: string }> {
    if (!value) return [];
    if (Array.isArray(value)) return value.flatMap((item) => this.getConstraintRefs(item));
  ...
  private offsetComponent(component: ResolvedComponent, offset: { x: number, y: number }): ResolvedComponent {
    ...
    if (shifted.pathSegments) {
      shifted.pathSegments = shifted.pathSegments.map((segment) => ({
        ...
        inP: segment.inP ? { x: segment.inP.x + offset.x, y: segment.inP.y + offset.y } : undefined,
        outP: segment.outP ? { x: segment.outP.x + offset.x, y: segment.outP.y + offset.y } : undefined,
      }));
    }
  ...
  private normalizeConstraints(node: ASTNode): Record<string, any> {
    ...
    if (node.properties.direction !== undefined && node.properties.angle === undefined) {
  ...
  private normalizeOnConstraint(rawOn: any): NormalizedOnConstraint | undefined {
    if (!rawOn) return undefined;
    if (rawOn.type === "anchor" && rawOn.id && rawOn.anchor) {
      return { anchorRef: rawOn };
    }
    if (typeof rawOn === "object" && rawOn.anchor && rawOn.anchor.type === "anchor") {
  ...
  private applyContactProperties(properties: Record<string, any>, onConstraint?: NormalizedOnConstraint): Record<string, any> {
    ...
    if (typeof onConstraint.mu === "number") {
      nextProps.contact_mu = onConstraint.mu;
    }
    return nextProps;
  }
}
